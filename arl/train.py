...
def train_model(
    ...
    auc = 0
    with torch.no_grad():
        test_logits, test_sigmoid, test_pred = model.learner(test_cat, test_num)
        auc = metrics.set_auc(test_sigmoid, test_target, n_iters)
                
    return test_pred,auc
...
#         seed_everything(42 + i)
    ...
    dpath = test_dataset.dataset_stats['path']

    try:
        os.mkdir(dpath+'preds/')
    except:
        pass
    
    torch.save(pred,dpath+'preds/ARL_pred'+str(uuid.uuid1())+'.pt')
#     torch.save(model.state_dict(), dpath+'preds/ARL_model.pt')
