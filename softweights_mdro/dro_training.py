...
def get_results_for_learning_rates(input_df,test_df,
                                    ...
                                    learning_rate_p_lists =[[0.001, 0.001, 0.001],[0.01, 0.01, 0.01],[0.1, 0.1, 0.1]], 
                                    num_runs=1,  #10, num of splits
                                    minibatch_size=None,  #1000
                                    num_iterations_per_loop=25,  #100
                                    num_loops=30, #30
    ...
    best_learning_rate_p_list = None
    ...
    return final_average_results_dict['test_predictions'],best_learning_rate_theta,best_learning_rate_lambda,best_learning_rate_p_list

def get_robust_constraints(df, phats, proxy_columns, label_column, max_diff=0.05, max_num_ptilde = 20, maximum_p_radius=[1,1,1], constraint = 'tpr_and_fpr'):
    ...
    if constraint == 'tpr':
        ...
        for i in range(len(proxy_columns)):
            ...
            for j in range(max_num_ptilde):
                ...
                weighted_tp = np.multiply(ptilde, tp)
                weighted_tpr = float(sum(weighted_tp)/sum(weighted_labels))
                new_robust_constraint_tpr = tpr_overall - weighted_tpr - max_diff
