...
def get_model_for_learning_rates(input_df,test_df, 
    ...
    results_dicts_runs = []
    t_split = time.time()
    train_df, val_df = train_test_split(input_df, test_size = 0.48)
    test_df = test_df
    ...
    print("Starting optimizing learning rate theta: %.3f, learning rate lambda: %.3f, learning rate W: %.3f" % 
          (learning_rate_theta, learning_rate_lambda, learning_rate_W))
    ...
    sw_model.build_train_ops(constraint=constraint, learning_rate_theta=learning_rate_theta, 
                             learning_rate_lambda=learning_rate_lambda, learning_rate_W=learning_rate_W, constraints_slack=constraints_slack)
...
def get_results_for_learning_rates(input_df,test_df, 
                                    feature_names, protected_columns, proxy_columns, label_column, 
                                    constraint = 'tpr', 
    ...
    # 10 runs with mean and stddev
    results_dicts_runs = []
    best_learning_rate_theta = None 
    best_learning_rate_lambda = None
    best_learning_rate_W = None
...
#         train_df, val_df, test_df = data.train_val_test_split(input_df, 0.6, 0.2, seed=seed_start+i)
    ...
    return final_average_results_dict['test_predictions'],best_learning_rate_theta,best_learning_rate_lambda,best_learning_rate_W
